...
def load_presets(path: str) -> pd.DataFrame:
...
def generate():
    print(f"Selected image: {selected_preset}")
    print(handler_source.img_path)
...
def handle_selected_image():
    selected_image = request.form.get('image')

    if selected_image:
        selected_preset = int(selected_image)
...
if __name__ == "__main__":
    ...
    # load presets
    try:
        presets = load_presets(path=os.path.join("src", "static", "presets", "presets.csv"))
